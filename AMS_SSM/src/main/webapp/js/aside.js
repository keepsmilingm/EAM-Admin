...
$('#scrapHerf').attr("href",ScrapUrl);
$('#allotHerf').attr("href",AllotUrl);
$('#InventoryHerf').attr("href",InventoryUrl);
    ...
    $.ajax({
        ...
        success: function (datas) {
            ...
            for(var i=0;i<jsonArr.length;i++){
                ...
                getMsg(jsonArr[i].des,jsonArr[i].sender,
                    jsonArr[i].receiverId,jsonArr[i].messageId,jsonArr[i].pic)
            }
        }
    });
}
...
var picArr=[];
picArr=(JSON.parse($.getCookie("userInfo")).pic).split(",");
console.log(picArr);
if(picArr.length>0){
    $('#asideIcon').attr("src","/AMS_SSM"+picArr[0]);
    $('#headIcon1').attr("src","/AMS_SSM"+picArr[0]);
    $('#headIcon2').attr("src","/AMS_SSM"+picArr[0]);
}
